...
function handleChoiceChange(e, choice) {
    ...
    if (groupBy !== null) {
        const groupHeader = getNearestElement(e.currentTarget, 'ul')
            .parentElement;
...
const clickOut = (e) => {
    if (e.target !== selectionEl && !selectionEl?.contains(e.target)) {
        openList = false;
...
function handleSelectGroupBy(e, groupHeader) {
    ...
    const groupContainer = e.currentTarget.parentElement.parentElement;
    groupContainer
